...
const float cEmpireExpansionManager::eMeanOneSystem = 14.0f;
const float cEmpireExpansionManager::eMaxMean = 60.0f;
const float cEmpireExpansionManager::eCyclesToApexColonies = 180.0f;

const float cEmpireExpansionManager::mMeanOneSystem = 12.0f;
const float cEmpireExpansionManager::mMaxMean = 40.0f;
const float cEmpireExpansionManager::mCyclesToApexColonies = 120.0f;

const float cEmpireExpansionManager::hMeanOneSystem = 8.0f;
const float cEmpireExpansionManager::hMaxMean = 30.0f;
const float cEmpireExpansionManager::hCyclesToApexColonies = 80.0f;
...
void cEmpireExpansionManager::Initialize() {
	...
	meanOneSystem = mMeanOneSystem;
	maxMean = mMaxMean;
	cyclesToApexColonies = mCyclesToApexColonies;
...
void cEmpireExpansionManager::OnModeEntered(uint32_t previousModeID, uint32_t newModeID) {
	if (newModeID == GameModeIDs::kGameSpace) {
		elapsedTime = 0;
		cPlayer* player = GameNounManager.GetPlayer(); // Maybe call ensurePlayer() first for safety? I tested it, and it doesn't seem necessary but idk
		Difficulty difficulty = player->mDifficultyLevel;
		switch (difficulty) {
		case kDifficultyEasy:
			meanOneSystem = eMeanOneSystem;
			maxMean = eMaxMean;
			cyclesToApexColonies = eCyclesToApexColonies;
			break;
		...
		case kDifficultyHard:
			meanOneSystem = hMeanOneSystem;
			maxMean = hMaxMean;
			cyclesToApexColonies = hCyclesToApexColonies;
			break;
		}
	}
...
void cEmpireExpansionManager::GeneratePlanets(cStarRecord* star) {
	/*for (cPlanetRecordPtr planet : star->GetPlanetRecords()) {
		/*StarManager.CalculatePlanetScores(planet.get(), star);
		float orbitDistance = GetSolarStarOrbitRadius(star->GetType(), GetPlanetOrbitTemperature(planet.get()));
		cEllipticalOrbit orbit = cEllipticalOrbit();
		StarManager.GenerateEllipticalOrbit(star, orbit, orbitDistance, orbitDistance);
		planet->mOrbit = orbit;
		if (planet->mType == PlanetType::T0 || 
			planet->mType == PlanetType::T1 ||
			planet->mType == PlanetType::T2 ||
			planet->mType == PlanetType::T3) {
			SpaceTrading.AssignPlanetSpice(planet.get());
				if (planet->mType == PlanetType::T0) {
					//cPlanetRecord::AssignTerrainNonT0(planet.get());
				}
		}
	}*/
...
void cEmpireExpansionManager::ExpandEmpire(cEmpire* empire) {
	...
	//The first four colonies are determined using a different method.
	if (empireStars.size() > 4) {
		...
		for (cStarRecordPtr empireStar : empireStars) {
			...
			GetUnclaimedStarsInRadius(empireStar->mPosition, nearStarRadius, nearUnclaimedStars);
			for (cStarRecordPtr nearStar : nearUnclaimedStars) {
				...
				if (ColonizableStar(nearStar.get()) && nearStar.get() != GetCurrentStar()) {
		...
		/*
		eastl::vector<cStarRecordPtr>homeworldCloserStars;
		if (candidateStar != NULL) {
			GetUnclaimedStarsInRadius(homeworld->mPosition, GetDistanceBetweenStars(homeworld, candidateStar.get()) / 2, homeworldCloserStars);
			for (cStarRecordPtr closerStar : homeworldCloserStars) { //last check, for a much closer system from the HW, don't care about its weight.
				if (ColonizableStar(closerStar.get()) && closerStar.get() != GetCurrentStar()) {
					candidateStar = closerStar;
					break;
				}
			}
		}*/
	}
	// The first four colonies are the closest colonizable stars to the homeworld.
	else {
		eastl::vector<cStarRecordPtr>homeworldCloseStars;
		GetUnclaimedStarsInRadius(homeworld->mPosition, 7, homeworldCloseStars);
		float minDistance = 99;
		for (cStarRecordPtr closeStar : homeworldCloseStars) {
			StarManager.RequirePlanetsForStar(closeStar.get());
			float currentDistance = GetDistanceBetweenStars(homeworld, closeStar.get());
			if (ColonizableStar(closeStar.get()) && closeStar.get() != GetCurrentStar()) {
				if (minDistance > currentDistance) {
					candidateStar = closeStar;
